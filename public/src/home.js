...
function getMostCommonGenres(books) {
  const result = books.reduce((accum, book) => {
    ...
    []);
  result.sort((genreA, genreB) => sortBy(genreA.count, genreB.count, false));
  result.splice(5);
  return result;
}
...
function getMostPopularBooks(books) {
  ...
  result.sort((titleA, titleB) => sortBy(titleA.count, titleB.count, false));
  result.splice(5);
  return result;
}
...
function getMostPopularAuthors(books, authors) {
  ...
  result.sort((authorA, authorB) => sortBy(authorA.count, authorB.count, false));
